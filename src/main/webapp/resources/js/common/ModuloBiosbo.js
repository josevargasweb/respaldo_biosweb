...
class BiolisBuscadorOrdenes {
    ...
    for (let index in this.inputFields) {
      ...
      if (index !== jqid1 && index !== jqid2) {
        ...
        this.jqueryObjectList.forEach((jqObj) => {
          if (jqObj !== $(jqid1) && jqObj !== $(jqid1)) { jqObj.val("") };
        });
      }
      else {
        $(index).prop("readonly", !on);
  ...
  boFnFillSelect() {
    ...
    if (this.hfields["#bo_div_procedencia"] === true) {
    ...
    if (this.hfields["#bo_div_seccion"] === true) {
    ...
    if (this.hfields["#bo_div_servicio"] === true) {
      ...
      this.listaTiposDocumento.getTiposDocumento(this.boGenFillSelectFields("#bo_tipoDocumento", "ldvtdDescripcion", "ldvtdIdtipodocumento"));
    };
    if (this.hfields["#bo_div_nroDocumento"] === true) {
