...
$("#btnExamenesDerivadosExcel").click(function (){	
	$("#bo_nOrdenIni").prop('disabled', true);
	$("#bo_nOrdenFin").prop('disabled', true);
	$("#bo_examen").prop('disabled', true);

	$("#btnEstadisticaExamenesDerivadosExcel").removeAttr('hidden');
	$("#btnEstadisticaTotalPorExamen").prop('hidden', 'true');
	$("#btnEstadisticaTotalPorExamenExcel").prop('hidden', 'true');
	$("#btnEstadisticaTiempoEntrega").prop('hidden', 'true');
	$("#btnEstadisticaMuestrasRechazadas").prop('hidden', 'true');
...
$("#queryExamenesDerivados").click(function (){

	html = "";
	html += "<h3 id='' class='text-center' id='obsExamenModalLabel'>SQL Exámenes Derivados </h3> "
	html += `<label>select dfe.dfe_norden , ce.ce_abreviado, dfm.dfm_codigobarra as mr_codigobarras ,  cderiv.cderiv_descripcion, 
                ...
                inner join datos_fichasexamenes dfe  on dfe.dfe_norden = df.df_norden 
                 left join datos_fichasmuestras dfm  on dfm.dfm_idmuestra = 
                 (select dfet.dfet_idmuestra from datos_fichasexamenestest dfet where dfet.dfet_norden = dfe.dfe_norden and dfet.dfet_idexamen = dfe.dfe_idexamen FETCH FIRST 1 ROW only) 
              ...
              inner join cfg_secciones csec on csec.csec_idseccion = ce.ce_idseccion 
               ...
               inner join cfg_derivadores cderiv  on cderiv.cderiv_idderivador = dfe.dfe_idderivado       
              </label>
              <br>
...
$("#btnEstadisticaExamenesDerivadosExcel").click(async function() {	
...
let fechaIni=""
	...
	let data = await  getExamenesDerivadosDescarga(boFormData);	
	
   llenarTablaExamenesDerivados(data);	
   	  $("#estadisticaHistorico").css("display", "none");
	 $("#modalFiltroEstadistica").modal("hide")
		agregarExcel("Exámenes Derivados")
		limpiar;
...
$("#btnEstadisticaExamenesDerivados").click(async function() {		
	let fechaIni=""
	let fechaFin =""
	if($("#bo_fIni").val() != ""){
		 fechaIni = convertirDateDDMMYYYY(new Date($("#bo_fIni").val() + 'T00:00:00'))
	...
	let data = await  getExamenesDerivados(boFormData);	
...
async function getExamenesDerivados(data) {
	 let url= getctx +"/api/estadistica/ExamenesDerivados";
...
async function getExamenesDerivadosDescarga(data) {
	 let url= getctx +"/api/descarga/estadistica/ExamenesDerivados";
	 let postData=JSON.stringify(data)
     let result = await $.post({
		type: "post",
		url : url,
...
async function llenarTablaExamenesDerivados(data) {	
let fechaFin ="" ;
	let fechaIni = " ";
	if($("#bo_fFin").val() != ""){
		 fechaFin = convertirDateDDMMYYYY(new Date($("#bo_fFin").val()+'T00:00:00'))
...
let html = `	
		...
		<div class="card h-100">
			<div class="card-body motor-busqueda overflow-hidden">
				<div class="col-12 pl-0 pr-0">
					...
					<button  id="btnEstadisticaExamenesDerivados1"class="btn btn-blue-primary btn-lg font-weight-bold mr-2 ">
				...
				<h3 id="tituloBusqueda" class="col-12 pl-0 pr-0 text-center">Estadistica Exámenes derivados </h3>
	...
	data.forEach(function (dato) {
		html += `<tr Style=' font-size: 80%;'>
				<td>${dato.dfe_NORDEN}</td>
				<td>${dato.ce_ABREVIADO}</td>
				<td>${dato.mr_CODIGOBARRAS === null || dato.mr_CODIGOBARRAS ===" " ? "----------------" : dato.mr_CODIGOBARRAS}</td>
				<td>${dato.mr_DERIVADOR === null ? "----------------" : dato.cderiv_DESCRIPCION}</td>
				<td>${dato.fechatm === null ? "----------------" : dato.fechatm}</td>
			</tr>`;
	});
	...
	html += `		</tbody>
			...
			html += `			
				<table id="tb" class="table table-striped table-bordered display nowrap" width="80%" style='display:none'>
					...
					<th></th>
					<th></th>
						<th class="text-center">Exámenes Derivados</th>
					</tr>
					<tr></tr>
					<tr>
	...
	data.forEach(function (dato) {
		...
		html += "<tr Style=' font-size: 60%;'>"		
						...
						html +="<td>"+dato.dfe_NORDEN+"</td>"		
						html += "<td>"+dato.ce_ABREVIADO+"</td>"
						
						if(dato.mr_CODIGOBARRAS=== null || dato.mr_CODIGOBARRAS ===" "){
							html += "<td>----------------</td>"
						}else{
						...
						if(dato.mr_DERIVADOR === null){
							html += "<td>------------</td>"
						}else{
							html += "<td>"+dato.cderiv_DESCRIPCION+"</td>"
						}
						
						
						
						if(dato.fechatm === null){
							html += "<td>------------</td>"
						}else{
							html += "<td>"+dato.fechatm+"</td>"
						}
	...
	$("#btnEstadisticaExamenesDerivados1").click(async function() {
