...
$("#btnResultadosCriticosExcel").click(function (){	
	$("#bo_nOrdenIni").prop('disabled', true);
	$("#bo_nOrdenFin").prop('disabled', true);
	$("#bo_examen").prop('disabled', true);
	
	$("#btnEstadisticaResultadosCriticosExcel").removeAttr('hidden');
	$("#btnEstadisticaTotalPorExamen").prop('hidden', 'true');
	$("#btnEstadisticaTotalPorExamenExcel").prop('hidden', 'true');
	$("#btnEstadisticaTiempoEntrega").prop('hidden', 'true');
	$("#btnEstadisticaMuestrasRechazadas").prop('hidden', 'true');
...
$("#queryResultadosCriticos").click(function (){
	html = "";
	html += "<h3 id='' class='text-center' id='obsExamenModalLabel'>SQL Resultados Criticos </h3> "
	html += `<label>select ct.ct_codigo as codigo,  ct.ct_abreviado as testabreviado, dfet.dfet_resultado as resultado, dfet.dfet_valorcriticohasta as valorcriticohasta , 
...
$("#btnEstadisticaResultadosCriticosExcel").click(async function() {
	...
	if(validarDatosVacios() === true){
		...
		let data = await  getResultadosCriticosDescarga(boFormData);	
		
	 	llenarTablaResultadosCriticos(data);
	 	    $("#estadisticaHistorico").css("display", "none");
		$("#modalFiltroEstadistica").modal("hide")
			
			agregarExcel("Resultados criticos")
			limpiar;
...
$("#btnEstadisticaResultadosCriticos").click(async function() {
	...
	if(validarDatosVacios() === true){
		 ...
		 let boFormData = new Object();
	      ...
	      boFormData.bo_examen =  $("#bo_examen option:selected").val(); 
			...
			let data = await  getResultadosCriticos(boFormData);	
...
async function getResultadosCriticos(data) {
	 let url= getctx +"/api/estadistica/resultadoscriticos";
...
async function getResultadosCriticosDescarga(data) {
	 let url= getctx +"/api/descarga/estadistica/resultadoscriticos";
	 let postData=JSON.stringify(data)
     let result = await $.post({
		type: "post",
		url : url,
...
async function llenarTablaResultadosCriticos(data) {	
	let fechaFin ="" ;
	let fechaIni = " ";
	if($("#bo_fFin").val() != ""){
		 fechaFin = convertirDateDDMMYYYY(new Date($("#bo_fFin").val()+'T00:00:00'))
...
let html = `	
		...
		<div class="card h-100">
			<div class="card-body motor-busqueda overflow-hidden">
				<div class="col-12 pl-0 pr-0">
					...
					<button  id="btnEstadisticaResultadosCriticos1"class="btn btn-blue-primary btn-lg font-weight-bold mr-2 ">
				...
				<h3 id="tituloBusqueda" class="col-12 pl-0 pr-0 text-center">Estadistica Resultados Criticos</h3>
				...
				<tbody id="bodyTablaBusqueda">					
						...
						<th>Test </th> 						
						<th>Resultado</th>
						<th>Referencia crítico bajo</th>	
						<th>Referencia crítico alto</th>				    
				        <th>Conclusión' </th>
	...
	data.forEach(function (dato) {
		...
		html += "<tr Style=' font-size: 80%;'>"		
						...
						html +="<td>"+dato.codigo+"</td>"
						if(dato.testabreviado === null){
							html +="<td>--------------</td>"
						}else{
							html +="<td>"+dato.testabreviado+"</td>"
						}
						if(dato.resultado === null){
							html +="<td></td>"
						}else{
							html +="<td>"+dato.resultado+"</td>"
						}				
						if(dato.valorcriticodesde === null){
							html +="<td> </td>"
						}else{
							html +="<td>"+dato.valorcriticodesde+"</td>"
						}
						if(dato.valorcriticohasta === null){
							html +="<td> </td>"
						}else{
							html +="<td>"+dato.valorcriticohasta+"</td>"
						}
						if(dato.conclusion === null){
							html +="<td>  </td>"
						}else{
							html +="<td>"+dato.conclusion+"</td>"
						}
						if(dato.forden === null){
							html +="<td></td>"
						}else{
							html +="<td>"+dato.forden+"</td>"
						}
						if(dato.ffirma === null){
							html +="<td></td>"
						}else{
							html +="<td>"+dato.ffirma+"</td>"
						}
	...
	html += `	</tr></tbody>
			...
			html += `			
				<table id="tb" class="table table-striped table-bordered display nowrap" width="80%" style='display:none'>
					...
					<th></th>
					<th></th>
						<th class="text-center">Resultados Criticos</th>
					</tr>
					<tr></tr>
	...
	$("#btnEstadisticaResultadosCriticos1").click(async function() {
