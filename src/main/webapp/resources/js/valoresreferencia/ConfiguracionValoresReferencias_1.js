...
$(document).ready(function(){
    ...
    $("#btnGuardar").click(function (){
        //e.preventDefault();
        let indx = tableValoresReferencia.rows().data();
        let items = new Array();
...
function selectCallback(data){
    $("#divVR").show();
    testId = data.cfgTest.ctIdtest;
    if (!$.fn.dataTable.isDataTable( '#tablaValoresReferencia')) {
        initDatatable(testId);
    } else {
        let newUrl = getctx + "/api/valoresReferencia/list/"+testId;
        tableValoresReferencia.ajax.url(newUrl).load();
    }
    $.ajax({
        type: "GET",
        url: getctx + "/api/metodos/test/" + testId,
        datatype: "json",
        async: false,
        success: function(data){
...
//initDataTable(){
        ...
        $('#tablaValoresReferencia tbody').on('blur', 'input', function (e) {
            ...
            /*
            if (validaDatos(value, rowName, dataId, fila)){
                $(input).prop('disabled', true);
                $(this).parent().parent().find('a').show();
                fila[rowName] = value;
            } else {
                handlerMessageError("Los rangos no deben coinicidir con otros valores de referencia")
                tableValoresReferencia.ajax.reload(null, false);
            }*/
        });
